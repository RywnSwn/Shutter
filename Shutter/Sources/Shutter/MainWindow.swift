...
struct MainWindow: View {
    ...
    // Session-scoped unlock for editing the Apps/Websites lists while Secure Mode is on.
    // Resets whenever the window closes — re-opening always re-prompts.
    @State private var listsUnlocked: Bool = false

    enum Tab: Hashable { case apps, websites, settings }

    private var listsLocked: Bool {
        state.isSecured && KeychainStore.hasPassword && !listsUnlocked
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            TabView(selection: $selectedTab) {
                AppPickerList(installed: filtered, selected: bindingSelected, search: $search)
                    .modifier(ListsLockOverlay(isLocked: listsLocked, onUnlock: unlockLists))
        ...
        .onChange(of: state.isSecured) { secured in
            // No reason to keep the lock state hanging around once Secure Mode is off.
            if !secured { listsUnlocked = false }
        }
    }

    private func unlockLists() {
        PasswordPrompt.verify(reason: "Enter your password to edit blocked apps and websites.") { ok in
            if ok { listsUnlocked = true }
        }
    }
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 16) {
            ...
            Spacer()
            // Visual is bound to a constant so SwiftUI never animates it ahead of the
            // real state. The tap is handled by an invisible overlay button that calls
            // the auth check first, and only the published `isSecured` flip moves the
            // switch — no stale-position flicker on biometric cancel.
            Toggle("", isOn: .constant(state.isSecured))
                ...
                .overlay(
                    Button(action: requestToggle) { Color.clear }
                        .buttonStyle(.plain)
                )
        }
...
/// Frosted scrim shown over the Apps/Websites tab content while Secure Mode is on
/// and a password is set. Prevents the obvious bypass of just emptying the lists from
/// Settings. One password entry unlocks both tabs for the session; closing the window
/// re-locks.
private struct ListsLockOverlay: ViewModifier {
    let isLocked: Bool
    let onUnlock: () -> Void
                ...
                if isLocked {
                    ZStack {
                        ...
                        VStack(spacing: 14) {
                            ZStack {
                                Circle()
                                    .fill(Color.shutterAccent.opacity(0.15))
                            ...
                            VStack(spacing: 6) {
                                Text("Locked while Secure Mode is on")
                                    .font(.system(size: 14, weight: .semibold))
                                Text("Enter your password to edit blocked apps and websites. Closing this window re-locks.")
                                    .font(.system(size: 12))
                                    .foregroundStyle(.secondary)
